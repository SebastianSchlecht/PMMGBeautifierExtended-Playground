...
export function ProcessMessage (eventData: string) {
    ...
    if ("messageType" in parsedData && "payload" in parsedData) {
        ...
        try {
            success = USED_MESSAGE_TYPES[parsedData["parsedData"]]?.(parsedData["payload"]) ?? null;
        } catch (e) {
            success = false;
        }

        if (!receivedMsgTypes.has(parsedData["messageType"])) {
            receivedMsgTypes.add(parsedData["messageType"]);
            console.log("Received new msg type", parsedData["messageType"], "will only log once");
        }
...
function processContract(payload2: any): boolean {
